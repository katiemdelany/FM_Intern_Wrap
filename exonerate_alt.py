...
def protein_sort(records):
    ...
    for contig in records:
        ...
        if protein in proteinHits:
			# append to the dictionary the informationa bout the hit sequences from exonerate
			...
			# if is the first start the dictionary (I guess)
...
def sort_key(elem):
    ...
    ### modified by Claudio (depth has been deleted to be able to use assemblies not produced by SPAdes)
    # original lines taking into account the coverage value from spades header to order the hits
    #return elem[0],elem[1],-elem[2]
    # same thing but not using it...
...
def get_contig_order(prot):
    ...
    logger = logging.getLogger("pipeline")
    ### modified by Claudio
    # original line splitting the headers using "_", assuming it is a spades style header to get element five: the coverage value (float number)
    #tuplist =[(prot["hit_start"][i],prot["hit_end"][i],float(prot["assemblyHits"][i].split(",")[0].split("_")[5])) for i in range(len(prot["hit_start"]))]
    # same thing without using SPAdes style headers...
